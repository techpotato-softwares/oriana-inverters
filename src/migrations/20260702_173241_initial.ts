...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`_products_v_version_key_specs\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`label\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_products_v_version_key_specs_order_idx\` ON \`_products_v_version_key_specs\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_products_v_version_key_specs_parent_id_idx\` ON \`_products_v_version_key_specs\` (\`_parent_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`media\` (
  	...
  	\`sizes_4k_mime_type\` text,
  	\`sizes_4k_filesize\` numeric,
  	\`sizes_4k_filename\` text
  );
  `)
  ...
  await db.run(sql`CREATE INDEX \`media_sizes_4k_sizes_4k_filename_idx\` ON \`media\` (\`sizes_4k_filename\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`_products_v_version_key_specs\`;`)
